...
from DeluxeMod.Entities.TelegramEntity import TelegramEntity
...
class BaseMatch:
    ...
    async def init_async(self):
        ...
        @RegisterEvent(self.session.id, event=ChooseActionEvent)
    ...
    async def send_message_to_player(self, text: Union[str, LocalizedString], player: TelegramEntity,
        ...
        for tts in smartsplit.smart_split(text):
            try:
                ...
                if (not ignore_dm_game and self.chat_id == player.user_id) or player.type == 'npc':
                    return
    ...
    async def choose_actions(self):
        """Executes actions for all alive entities."""
        for entity in self.session.alive_entities:
            choose_action_event = ChooseActionEvent(self.session.id, self.session.turn, entity.id)
    ...
    async def attempt_finish_weapon_choice(self):
        if len(self.chosen_weapon) == len(self.session.entities):
